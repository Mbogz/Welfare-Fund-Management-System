...
import { Layers, ShieldCheck, Banknote, Clock, RefreshCw } from 'lucide-react';
...
export default function SuperAdminDashboard() {
  ...
  const fetchLiveDashboardData = async () => {
    setLoading(true);
    try {
      // 1. Get Live Group Count from Supabase
      const { count: groupsCount } = await supabase
        .from('groups')
        .select('*', { count: 'exact', head: true });

      // 2. Get Live Admin Count from profiles
      const { count: adminsCount } = await supabase
        .from('profiles')
        .select('*', { count: 'exact', head: true })
        .eq('role', 'admin');

      // 3. Get Recent Activity Logs Feed
      ...
      // Setting a placeholder platform volume for now until transactions table is live
      setTotalRevenue(0); 

    } catch (error) {
      console.error("Failed to sync dashboard numbers:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 mt-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 pb-5 gap-4">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div className="space-y-2">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block">Total Platform Vol.</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {loading ? (
            <div className="p-8 text-center text-sm text-gray-400">Fetching live server cluster timeline...</div>
          ) : recentActivity.length === 0 ? (
            <div className="p-8 text-center text-sm text-gray-400">No recent deployment activity on record.</div>
          ) : (
            recentActivity.map((activity) => (
              <div key={activity.id} className="p-5 flex items-center justify-between hover:bg-gray-50/30 transition-colors">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Pre-authorized system profile slot as <span className="font-medium text-gray-600 capitalize">{activity.role}</span>
                    </p>
                  </div>
                </div>
                <span className="text-xs font-mono text-gray-400">
                  {activity.joined_at ? new Date(activity.joined_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : 'Pending'}
                </span>
              </div>
            ))
          )}
